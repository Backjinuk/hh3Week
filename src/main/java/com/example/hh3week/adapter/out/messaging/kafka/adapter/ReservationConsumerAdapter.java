...
public class ReservationConsumerAdapter {
	...
	private final ResponseHolder requestHolder;
	...
	@Autowired
	public ReservationConsumerAdapter(ReservationUseCase reservationUseCase,  ReservationMessagingPort reservationMessagingPort,
		OutboxEventRepository outboxEventRepository, ResponseHolder requestHolder) {
	...
	public void consumeIssuedTokenResponse(TokenIssuedResponse response){
		...
		CompletableFuture<TokenDto> future =  requestHolder.getResponse(correlationId);
		if (future != null) {
			if (tokenDto != null) {
